...
class ShowNotes:
    ...
    def read_file(self, filename: str) -> ShowNotes:
        ...
        with tempfile.NamedTemporaryFile() as temporary_input_metadata:
            os.system(f"sed '1d;/---/,$d' {filename} >| {temporary_input_metadata.name}")
        ...
        metadata["slug"] = "/posts/" + subprocess.run(["osmosisKebabCase", metadata["title"]], capture_output=True, text=True).stdout.strip()
        metadata["description"] = subprocess.run(["osmosisExtractDescription", filename], capture_output=True, text=True).stdout.strip()
        metadata["body"] = subprocess.run(["osmosisExtractBody", filename], capture_output=True, text=True).stdout.strip()
...
class PrepareAudio:
    ...
    def __init__(self, source_filename: str) -> None:
        ...
        self.size = int(subprocess.run(["osmosisRetrieveAudioMetadata", "-s", self.source_filename], capture_output=True, text=True).stdout.strip())
        self.duration = int(subprocess.run(["osmosisRetrieveAudioMetadata", "-d", self.source_filename], capture_output=True, text=True).stdout.strip())
        return None

    def apply_audio_metadata(self, title: str, date: str) -> None:
        exit_status = os.system(f"osmosisApplyAudioMetadata {self.source_filename} \"{title}\" \"{date}\" ")
        if exit_status != 0:
            raise ValueError(f"Unable to apply audio metadata to {self.source_filename}")
    ...
    def write_to_r2(self, season_number: int, episode_number: int) -> str:
        ...
        upload_filepath = os.path.join(bucket, season_string, episode_string, local_filepath)
        audio_url = os.path.join(domain, season_string, episode_string, local_filepath)

        exit_status = os.system(f"npx wrangler r2 object put --file {self.source_filename} \"{upload_filepath}\"")
        if exit_status == 0:
            return audio_url
        else:
            raise ValueError(f"{upload_filepath} failed to upload to R2 with this exit code {exit_status}")
...
class PrepareEpisodePage:
    ...
    def __init__(self, show_notes: ShowNotes, audio_filename: str, output_directory: Optional[str]) -> None:
        ...
        if output_directory is None:
            ...
            title = subprocess.run(["osmosisKebabCase", self.show_notes.metadata["title"]], capture_output=True, text=True).stdout.strip()

            website_repository_directory = os.path.expanduser("~/Documents/Development/osmosis-website/content/posts")
            content_directory = f"{date}-{season}-{episode}-{title}"

            self.output_directory = os.path.join(website_repository_directory, content_directory)
        else:
...
def main() -> None:
    ...
    audio.apply_audio_metadata(show_notes.metadata["title"], show_notes.metadata["date"])

    if arguments.output_directory is None:
        audio_filename = audio.write_to_r2(show_notes.metadata["season"], show_notes.metadata["number"])
    if arguments.output_directory is not None:
        audio_filename = arguments.source_audio

    show_notes.metadata["url"] = audio_filename
    episode_page = PrepareEpisodePage(show_notes=show_notes, audio_filename=audio_filename, output_directory=arguments.output_directory)
